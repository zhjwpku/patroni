...
class Cluster(NamedTuple('Cluster',
    ...
    :ivar workers: dictionary of workers of the MPP cluster, optional. Each key is an :class:`int` representing the
                   group (Note: key type may change to :class:`Any` when more MPP databases are introduced), and the
...
class AbstractDCS(abc.ABC):
    ...
    def _mpp_cluster_loader(self, path: Any) -> Dict[int, Cluster]:
        """Load and build all PostgreSQL clusters of a single MPP cluster.
    ...
    def is_mpp_coordinator(self) -> bool:
        """:class:`Cluster` instance has a MPP Coordinator group ID.

        :returns: ``True`` if the given node is running as MPP Coordinator.
        """
